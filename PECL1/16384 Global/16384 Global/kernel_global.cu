...
*/
{
	...
	dim3 matriz_hilos(dimMatriz.numFilas, dimMatriz.numColumnas);

	// Primero, copio salida a entrada. 
	kernelCopiarMatriz << < 1, matriz_hilos >> > (d_matrizCopia, d_matrizPega, d_dimMatriz);
	cudaDeviceSynchronize();

	// Despu�s, pongo a 0 la matriz de copia. 
	kernelSetMatrizCeros << < 1, matriz_hilos >> > (d_matrizCopia, d_dimMatriz);
	cudaDeviceSynchronize();
...
__host__ bool desplazarMatriz(int *h_matrizEntrada, int *h_matrizSalida, int* d_matrizEntrada, int* d_matrizSalida, int* h_puntuacion, int despVertical, int despHorizontal)
{
	...
	// Defino tamano de bloques:
	dim3 matriz_hilos(dimMatriz.numFilas, dimMatriz.numColumnas);

	// Realizo la suma:
	kernelSuma << < 1, matriz_hilos >> > (d_matrizEntrada, d_matrizSalida, d_puntuacion, d_despVertical, d_despHorizontal, d_dimMatriz);
	...
	{
		// Primero, copio salida a entrada. 
		kernelCopiarMatriz << < 1, matriz_hilos >> > (d_matrizSalida, d_matrizEntrada, d_dimMatriz);
		cudaDeviceSynchronize();

		// Segundo, seteo salida a 0.
		kernelSetMatrizCeros << < 1, matriz_hilos >> > (d_matrizSalida, d_dimMatriz);
		cudaDeviceSynchronize();

		// Desplazo
		kernelDesplazar << < 1, matriz_hilos >> > (d_matrizEntrada, d_matrizSalida, d_despVertical, d_despHorizontal, d_dimMatriz);
		cudaDeviceSynchronize();
		...
		// Veo si son iguales.
		kernelComprobarIguales << < 1, matriz_hilos >> > (d_matrizSalida, d_matrizEntrada, d_iguales, d_dimMatriz);
		cudaDeviceSynchronize();
...
__host__ bool estaLlena(int* d_matriz)
{
	...
	// Veo si está llena.
	kernelComprobarLlena << < 1, matriz_hilos >> > (d_matriz, d_estaLlena, d_dimMatriz);
	cudaDeviceSynchronize();
...
__host__ bool finJuego(int* d_matriz)
{
	...
	// Veo si está llena.
	kernelComprobarSiHaGanado << < 1, matriz_hilos >> > (d_matriz, d_haGanado, d_dimMatriz);
	cudaDeviceSynchronize();
...
*/
{
	...
	kernelComprobarMovimientosPosibles << < 1, matriz_hilos >> > (d_matriz, d_movimientosPosibles, d_dimMatriz);
	cudaDeviceSynchronize();
